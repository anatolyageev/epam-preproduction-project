...
public class SqlQuery {
    ...
    public static final String SQL_PRODUCT_GET_ALL = "SELECT pr.id, category_name, producer_name, name, short_description, price, image, meta_title  FROM computer_shop.products pr left join computer_shop.categories c ON pr.category_id = c.id left join computer_shop.producers p ON pr.producer_id = p.id";
    ...
    public static final String SQL_GET_ONE_PRODUCT = "SELECT pr.id, category_name, producer_name, name, short_description, price, image, meta_title FROM computer_shop.products pr left join computer_shop.categories c ON pr.category_id = c.id left join computer_shop.producers p ON pr.producer_id = p.id WHERE pr.id = ?";
